(function (c) {
    ...
    var d = (function () {
        ...
        var k = function (w, r, q, s, v, u) {
            var t = f();
            t.push({id: w, name: r, summary: q, price: s, quantity: v, image: u});
            n(t)
        };
        var f = function () {
            try {
    ...
    var a = function (j) {
        ...
        if (i.cartItems && i.cartItems.constructor === Array) {
            ...
            c.each(i.cartItems, function () {
                d.setProduct(this.id, this.name, this.summary, this.price, this.quantity, this.image)
            })
        }
        ...
        var z = function () {
            ...
            c.each(C, function () {
                ...
                A.append('<tr title="' + this.summary + '" data-id="' + this.id + '" data-price="' + this.price + '"><td class="text-center" style="width: 30px;"><img width="30px" height="30px" src="' + this.image + '"/></td><td>' + this.name + '</td><td title="Unit Price">' + i.currencySymbol + e.getRoundedNumber(this.price) + '</td><td title="Quantity"><input type="number" min="1" style="width: 70px;" class="' + v + '" value="' + this.quantity + '"/></td><td title="Total" class="' + g + '">' + i.currencySymbol + e.getRoundedNumber(D) + '</td><td title="Remove from Cart" class="text-center" style="width: 30px;"><a href="javascript:void(0);" class="btn btn-xs btn-danger ' + h + '">X</a></td></tr>')
            });
            A.append(C.length ? '<tr><td></td><td><strong>Total</strong></td><td></td><td></td><td><strong id="' + f + '"></strong></td><td></td></tr>' : '<div class="alert alert-danger" role="alert" id="' + x + '">Your cart is empty</div>');
            var B = i.getDiscountPrice(C, d.getTotalPrice(), d.getTotalQuantity());
            if (C.length && B !== null) {
        ...
        var s = function () {
            c("#" + f).text(i.currencySymbol + e.getRoundedNumber(d.getTotalPrice()))
        };
        var n = function () {
            c("#" + w).text(i.currencySymbol + e.getRoundedNumber(i.getDiscountPrice(d.getAllProducts(), d.getTotalPrice(), d.getTotalQuantity())))
        };
        if (i.affixCartIcon) {
        ...
        r.click(function () {
            i.showCheckoutModal ? l() : i.clickOnCartIcon(r, d.getAllProducts(), d.getTotalPrice(), d.getTotalQuantity())
        });
        c(document).on("input", "." + v, function () {
            var A = c(this).closest("tr").data("price");
            var C = c(this).closest("tr").data("id");
            var B = c(this).val();
            c(this).parent("td").next("." + g).text(i.currencySymbol + e.getRoundedNumber(A * B));
            d.updatePoduct(C, B);
            p.text(d.getTotalQuantity());
            s();
            n()
        ...
        c(document).on("click", "." + q, function () {
            ...
            y();
            i.checkoutCart(d.getAllProducts(), d.getTotalPrice(), d.getTotalQuantity());
            d.clearProduct();
            p.text(d.getTotalQuantity());
            c("#" + t).modal("hide")
        });
        c(document).on("click", j, function () {
            var A = c(this);
            ...
            p.text(d.getTotalQuantity());
            i.afterAddOnCart(d.getAllProducts(), d.getTotalPrice(), d.getTotalQuantity())
        })
    };
